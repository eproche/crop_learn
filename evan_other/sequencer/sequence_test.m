...
if ~exist('svm_model','var')
    load(strcat('/stash/mm-group/evan/saved_models_box_adjust/',object_class,'_model_check.mat'),'svm_model');
end 
...
for i = 8:10%order of images to try
    disp(i);
...
%         if 1 == 1 %determine which objects to look at
...
%%
            class_guess = predict_crop_seq(svm_model,start_crop);
...
%% Detetection Loop
            while class_guess ~= 7; 
                ...
                [class_guess,name] = evan2max(class_guess);
                move_amount = r3(1);
...
%%               
                ...
                class_guess = predict_crop_seq(svm_model,new_crop);
                [class_guess,name] = evan2max(class_guess);
                if class_guess == 7
%%                   
