...
list2 = {{'right','down','right','expand'},{'right','right','up','expand'},{'right','down','shrink'},{'right','up','shrink'},...
{'left','down','shrink'},{'left','up','shrink'},{'left','left','up','expand'},{'left','left','down','expand'},...
{'up','up','left','expand'},{'up','up','right','expand'},{'down','down','left','expand'},{'down','down','right','expand'}};
...
for i = 1:len_folder%order of images to try
    ...
    for k = 1:num_objects 
        ...
        h = record(i).(box)(4);
        scale_max = 0.4;
        scale_min = 0.2  %translate by percent of relevant bounding box dimension 
        a = 0.3 ; %range for randomized primary shift
        b = 0.5;
        c = -0.05; %range for randomized secondary shift 
        d = 0.05;
        ...
        for j = 1:numel(start_list)
            ...
            seco = start_list{j}{2};
            fh2 = str2func(seco);
            third = start_list{j}{3};
            fh3  = str2func(third);
            if length(start_list{j}) == 4
                fourth = start_list{j}{4};
                fh4 = str2func(fourth);
            end
...
%% Apply the transformations
            ...
            if strcmp(seco,'shrink') == 1
                temp = fh2(x1,y1,w1*r5(1),h1*r5(2),r,r2,fli,small_w,small_h);
            else
                temp = fh2(x1,y1,w1*r5(3),h1*r5(4),r,r2,fli,big_w,big_h);
            end
            x1 = temp(1); y1 = temp(2); w1 = temp(3); h1 = temp(4);
            if strcmp(third,'shrink') == 1
                crop = fh3(x1,y1,w1*r5(1),h1*r5(2),r,r2,fli,small_w,small_h);
            else
                crop = fh3(x1,y1,w1*r5(3),h1*r5(4),r,r2,fli,big_w,big_h);
            end
            x1 = crop(1); y1 = crop(2); w1 = crop(3); h1 = crop(4);
            if exist(fourth)
                if strcmp(fourth,'shrink') == 1
                    crop = fh4(x1,y1,w1*r5(1),h1*r5(2),r,r2,fli,small_w,small_h);
                else
                    crop = fh4(x1,y1,w1*r5(3),h1*r5(4),r,r2,fli,big_w,big_h);
                end
            end

%             base_image = imread(image_list{i}); % display the crop
%             B0 = insertShape(base_image,'Rectangle',orig_box,'Color','red','LineWidth',3);
%             B1 = insertShape(base_image,'Rectangle',crop,'Color','blue','LineWidth',3);
%             imshow(B1);
...
save('-v7.3','/stash/mm-group/evan/matlab/episodes_34.mat','episodes');