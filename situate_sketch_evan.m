...
% [ workspace, d, p, run_data, visualizer_status_string ] = situate_sketch( im_fname, p, learned_stuff );
    ...
    % workspace
        ...
        workspace_entry_events = cell(p.num_iterations,5);
    ...
    for iteration = 1:p.num_iterations
        ...
        % update visualization and records
            ...
            if workspace_total_support_post > workspace_total_support_pre
                ...
                cur_total_support = workspace.total_support(cur_workspace_entry_index); %removed
                box_runs = evaluated_agent_snapshot.box_adjust;
                box_used = evaluated_agent_snapshot.box_adjust_used;
                workspace_entry_events (iteration,:) = { iteration, evaluated_agent_snapshot.interest, cur_internal_support, box_runs, box_used };
            end
...
function [agent_pool,d] = agent_evaluate_scout( agent_pool, agent_index, im, im_label, d, p, workspace )     
    ...
    % see if we can improve the initial box with some tweaking
        ...
        if p.use_box_adjust ...
        && cur_agent.support.internal > p.internal_support_threshold && cur_agent.support.internal < p.total_support_threshold_2


    
            cur_box_adjust_model        = d(di).learned_stuff.box_adjust_models;
            num_adjustment_iterations   = 9;
            [updated_box_xywh, box_adjust_iterations] = box_adjust.apply_box_adjust_models_mq( cur_box_adjust_model, cur_agent.interest, im, cur_agent.box.xywh, num_adjustment_iterations, ground_truth_box_xywh);
            % temp = load('/u/eroche/spec.mat','runs','added');
            % runs = temp.runs;
            % added = temp.added;
            % runs = [runs box_adjust_iterations];
            cur_agent.box_adjust = box_adjust_iterations;
            if updated_box_xywh ~= 0
        ...
        % save('/u/eroche/spec.mat','runs','added','-append');
        end
