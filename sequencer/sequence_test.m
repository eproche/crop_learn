function [] = sequence_test(object_class)
...
if ~exist('svm_model','var')
    load(strcat('svm_',object_class,'.mat'),'svm_model');
end 
...
for i = 1:10 %order of images to try
    ...
    for k = 1:num_objects 
        ...
        if strcmp(object_class,'dog') == 1 %determine which objects to look at
        ...
        elseif strcmp(object_class,gen{1}) == 1
            object = object_class;
        elseif strcmp(object_class,gen{2}) == 1
            object = object_class;
        else 
            continue
        end
...
%% Apply the two transformations
            ...
            if strcmp(seco,'shrink') == 1
                crop = fh2(x1,y1,w1,h1,r,r2,fli,small_w,small_h);
            else
            ...
            total_count = 0;
            old_IOU = bboxOverlapRatio(orig_box,crop);
%% Detetection Loop
            while class_guess ~= 7; 
                rng('shuffle');
                ...
                if loop_count == 6
                    r3 = r;
                    loop_count = 0;
                    crop(3) = crop(3)*(r4(1)+1);
                    crop(4) = crop(4)*(r4(2)+1);
                end
                if total_count == 40; %gives up after X tries
                    break
                end
                class_name = decode(class_guess)
                move_amount = r3(1)
                ...
                values = {'previous',strcat('IOU: ',num2str(old_IOU)),class_name,strcat('move_amount: ',num2str(move_amount)),...
                ...
                old_IOU = IOU;
                if class_guess == 7
                    total_count = strcat('DONE: total= ',num2str(total_count))
