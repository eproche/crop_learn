...
% check a little that sub directories are included
    ...
    if ~exist('learned_stuff','var')
        load('/stash/mm-group/evan/data/learned_stuff.mat');
    end
    learned_stuff.box_adjust_models
...
%% define experiment settings
    ...
    experiment_settings = [];
    experiment_settings.use_gui = 0;
    
    experiment_settings.title               = 'box_adjust';
    ...
    experiment_settings.results_directory = '/u/eroche/doublemove/';
...
% contiaining images and label files.
    ...
    situate_data_path = '/stash/mm-group/evan/data/PortlandSimpleDogWalking/';
%     try
...
%    *_split_02_test.txt, *_split_02_train.txt ...
    ...
    % split_arg = [];                   
    split_arg = experiment_settings.results_directory;                     
...
%% define situate parameters: shared
    ...
    for i = 1:length(p.situation_objects ), p.situation_objects_urgency_pre.(p.situation_objects{i}) = object_urgency_pre(i);  end
    for i = 1:length(p.situation_objects ), p.situation_objects_urgency_post.(p.situation_objects{i}) = object_urgency_post(i); end

    p.situation_objects_possible_labels =  {...
        {'dog-walker back', 'dog-walker front', 'dog-walker my-left', 'dog-walker my-right'},...
        {'dog back', 'dog front', 'dog my-left', 'dog my-right'}, {'leash-/', 'leash-\'}};
    p.possible_paths = { ...
    ...
    disp('boop')
        p.doublemove = 1;
        p.use_box_adjust = true; % based on Evan's classifier based move selection
        p.num_adjustment_iterations = 6;
        ...
        p.thresholds.total_support_provisional = .2; %(search continues)
...
% the rest will be ignored
    ...
    description = 'box adjust full 0.2 UP DOWN'
    temp = p;
...
%     
   ...
   %% run the experiment
    ...
    situate_experiment_helper(experiment_settings, p_conditions, situate_data_path, split_arg,learned_stuff);
