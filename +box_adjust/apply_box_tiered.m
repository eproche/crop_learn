...
%scale factor for contract and expand, decreases slightly when called
    ...
    ud = [7 1 2];
    se = [21 5 6];
    track_1 = [0,16,16,16,16,7,7,7,7,21,21,21,21];
    track_2 = [0,7,7,7,7,7,16,16,16,16,16,21,21,21,21,21];
    track_3 = [0,21,21,21,21,21,7,7,7,7,7,16,16,16,16,16];

    stop = 0;
    cur_iteration = 1;
    for cur_iteration = 1:17
...
%             svm_indices = cur_track(cur_iteration);      
        end
        posteriors = zeros(length(allsvm),3); % not currently used at all

        for mi = svm_indices

           [class_prediction,posteriors(mi,1:2)] = predict(  box_adjust_model.box_adjustment_models_1v1{object_index,mi}, cur_feature_vect  );
           posteriors(mi,3) = mi; 
           for idx = 1:length(class_prediction)
            ...
            elseif mi == svm_indices(1) 
                forced_choice = class_prediction{idx};
            elseif strcmp(class_prediction{idx},'centered') == 1 
                votes.centered = votes.centered + 1;
            end
%            votes.(class_prediction{:}) = votes.(class_prediction{:}) + 1;
        end
%         disp(posteriors);
        ...
        if cur_iteration == 1
            ...
            if posteriors(1,1) == 1 && posteriors(2,1) == 1 && rflag >= 2 && lflag >= 2
                predicted_alignment = 'centered';
            elseif vote.centered == 6
                predicted_alignment = 'centered';
            else
...
%                 predicted_alignment = 'centered';
%                 stop = 1;
%             elseif winning_inds(1) == 1 
%                 predicted_alignment = alignments{winning_inds(2)};
%             else
%                 predicted_alignment = alignments{winning_inds(1)};
%             end    
%             elseif strcmp(cur_track,'rl') == 1
%                 if votes.left > votes.right
%                     predicted_alignment = 'left';
%                 elseif votes.right > votes.left
%                     predicted_alignment = 'right';
%                 else
%                     predicted_alignment = 'expand';
%                     lflag = 2; rflag = 2;
%                 end
%             elseif strcmp(cur_track,'ud') == 1
%                 if votes.up > votes.down
%                     predicted_alignment = 'up';
%                 elseif votes.down > votes.up
%                     predicted_alignment = 'down';
%                 else
%                     predicted_alignment = 'expand';
%                     uflag = 2; dflag = 2;
%                 end
%             elseif strcmp(cur_track,'se') == 1
%                 if votes.contract > votes.expand
%                     predicted_alignment = 'contract';
%                 elseif votes.expand > votes.contract
%                     predicted_alignment = 'expand';
%                 else
%                     predicted_alignment = 'centered';
%                     uflag = 2; dflag = 2;
