...
%% gather some interesting facts
    ...
    for ii = 1:size(agent_records,3)
        ...
        for oi = 1:length(situation_objects)
            ...
            workspace_entry_event_inds_object_type    = strcmp(object_label,  agent_records{mi,fi,ii}(:,1) );
            workspace_entry_event_inds_over_threshold = ge( round(100*[agent_records{mi,fi,ii}{:,5}])/100, iou_threshold );
            ...
            if ~isempty(cur_obj_first_detection_ind)
                temp_detection_times(oi) = agent_records{mi,fi,ii}{cur_obj_first_detection_ind,1};
            end
        end
