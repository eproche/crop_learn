...
if ~exist( 'results_directory', 'var' ) || ~isdir(results_directory)
    ...
    results_directory = '/u/eroche/sit_results/';
...
%% reshaping the data
    ...
    for fi = 1:length(fn);
        ...
        for j = 1:length(temp_d.p_conditions)
            ...
            p_conditions_descriptions_temp{end+1} = temp_d.p_conditions.description;
            agent_records_temp{end+1} = temp_d.agent_records(j,:);
    ...
    agent_records = cell(num_methods, num_folds, num_images_per_fold );
    ...
    for ii = 1:cur_num_images_in_fold
        ...
        agent_records{mi,fi,ii} = agent_records_temp{   cur_fold_ind }{ii};
        ...
        for k = 1:size(agent_records{mi,1,ii},1)
            disp(i)
            if isempty(agent_records{mi,1,ii}{k,4})
                agent_records{mi,1,ii}{k,4} = 0;
            end        
            x = agent_records{mi,1,ii}{k,1};
            y = sum(cell2mat(agent_records{mi,1,ii}(1:k,4)));
            newsum = x + y;
            agent_records{mi,1,ii}{k,1} = newsum;
            w = agent_records{mi,1,ii}{k,1};
        end


        [unique_detection_labels,~,label_assignments] = unique( agent_records{mi,fi,ii}(:,2) );
        was_over_threshold = ge( [agent_records{mi,fi,ii}{:,3}], checkin_threshold );
        ...
        for label_type_ind = 1:length(unique_detection_labels)
            ...
            if ~isempty(first_detection_row)
                first_detections(label_type_ind) = agent_records{mi,fi,ii}{first_detection_row,1};
            ...
            final_IOUs(mi,fi,ii,label_type_ind) = agent_records{mi,fi,ii}{final_detection_row,3};
...
final_workspace_snippet = cell(size(agent_records));
ending_support = cell(1,size(agent_records,1));
for mi = 1:size(agent_records,1) % method
...
%% draw the figures
    ...
    % for mi = 1:num_methods
    for mi = find(strcmp(unique_descriptions,'salience, normals, learned mvn'));
