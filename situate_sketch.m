...
% [ workspace, records, situate_visualizer_return_status ] = situate_sketch( im_fname, p, learned_models );
    ...
    % initialize record keeping 
        ...
        records.agent_record_flat = cell(p.num_iterations,10); % iteration, agent_type, interest, internal_support, total_support, ground truth iou, box_r0rfc0cf
        %records.dist_struct               = [];
...
%             if strcmp(agent_pool(agent_index).interest,'leash') == 1 && length(workspace.labels) < 2
%                 continue
%             end
        ...
        % update records
            ...
            records.agent_record_flat(iteration,:) = { ...
                ...
                current_agent_snapshot.box_adjust_runs, ...
                current_agent_snapshot.box_adjust_used, ...
                current_agent_snapshot.box.r0rfc0cf };
...
function agent = agent_initialize(d,p)
    ...
    agent.box_adjust_runs      = [];
    agent.box_adjust_used      = [];
...
function [agent_pool,d] = agent_evaluate_scout( agent_pool, agent_index, p, workspace, d, im, label ) 
    ...
    % figure out the internal support
        ...
        % cur_agent.box.xywh = round(cur_agent.box.xywh);
        % if ~all(cur_agent.box.xywh > 0) || length(cur_agent.box.xywh) ~= 4;
        %     neg = find(cur_agent.box.xywh < 0);
        %     cur_agent.box.xywh(neg) = 1;
            
        % elseif cur_agent.box.r0rfc0cf(2) > size(im,1)
        %     cur_agent.box.r0rfc0cf(2) = size(im,1);
        % elseif cur_agent.box.r0rfc0cf(4) > size(im,2)
        %     cur_agent.box.r0c0rfcf(4) = size(im,2);
        % end

         % rounding to nearest .01 for consistency between display and internal behavior

        cur_agent.support.internal = internal_support_score_function( cur_agent.box.xywh );
%         disp(cur_agent.support.internal);
        
    % see if we can improve the initial box with some tweaking
        
        if p.use_box_adjust ...
        && cur_agent.support.internal > p.thresholds.internal_support && cur_agent.support.internal < p.thresholds.total_support_final
%             di = relevant_label_ind;
            cur_box_adjust_model = d(di).learned_stuff.box_adjust_models;

            if ~isfield(p,'num_adjustment_iterations')
                p.num_adjustment_iterations = 9;
            end
%             disp(p.doublemove)
            [updated_box_xywh, box_adjust_iterations] = box_adjust.apply_box_adjust_models_mq( cur_box_adjust_model, cur_agent.interest, im, cur_agent.box.xywh, p.num_adjustment_iterations, ground_truth_box_xywh);

            
            % get updated internal support score
            new_internal_support = internal_support_score_function(updated_box_xywh);
            cur_agent.box_adjust_runs = box_adjust_iterations;
            ...
            if new_internal_support > cur_agent.support.internal
                ...
                if ~all(([x y w h]) > 0)
                    tempbox = [x y w h];
                    tempbox(tempbox < 0) = 1;
                    x = tempbox(1); y = tempbox(2); w = tempbox(3); h = tempbox(4);
                end
                r0 = y;
                rf = r0 + h - 1;
                c0 = x;
                ...
                xc = round(x + w/2);
                yc = round(y + h/2);
                cur_agent.box.xywh     = round([x y w h]);
                cur_agent.box.xcycwh   = round([xc yc w h]);
                cur_agent.box.r0rfc0cf = round([r0 rf c0 cf]);
                cur_agent.support.internal = new_internal_support;
                cur_agent.box_adjust_used = 1;
            else
                cur_agent.box_adjust_used = 0;
