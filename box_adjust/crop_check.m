...
if ~exist('net','var')
    global net layer;
    here = pwd;
    cd '/stash/mm-group/evan/cnn/matconvnet/';
...
for ii = 1:9 % alloted number of adjustments
    rng('shuffle'); 
    r4 = (ratio_max-ratio_min).*rand(4,1)+ratio_min;
    name = decode(class_guess) %uses old number encoding
    move_amount = r3(1);
...
%%
        ...
        IOU_idx = IOU_idx + 1;
        loop_scale = 0.2;
        r_f = crop(2) + crop(4) - 1; % convert back to situate box format
        c_f = crop(1) + crop(3) - 1;
